...
via `module.exports = logger`. Source: `src/utils/logger.js` line 127.
...
- The instance carries an attached `stream` property — an object of the
  ...
  Morgan-compatible. Source: `src/utils/logger.js` lines 110-121.
...
- Source: `src/utils/logger.js` lines 34-92 for the full Winston
...
- `winston` (declared in `/package.json` `dependencies`) — consumed only by
  `src/utils/logger.js` at line 22. The pinned semver range is captured in
  ...
  `src/utils/logger.js` line 22, `/package.json`.
...
5. Winston routes the `http`-level entry through the base format pipeline
   ...
   - `logs/combined.log` — accepted because the transport's
     `level: 'http'` matches. Source: `src/utils/logger.js` line 65.
...
2. Winston dispatches the entry to its transports, filtered by each
   ...
   - `debug` is emitted by the root logger only when `config.logLevel`
     ...
     transport. Source: `src/utils/logger.js` lines 39, 65.
3. All file-transport output is JSON-serialized and decorated with the
   ...
   property. Source: `src/utils/logger.js` lines 45-49, 53.
...
- `logger.js` reads `config.logLevel` once at module load and uses it as
  the root Winston logger level. Source: `src/utils/logger.js` line 39;
...
- `'logs/combined.log'`. Source: `src/utils/logger.js` line 64.
- `'logs/error.log'`. Source: `src/utils/logger.js` line 75.
...
- Winston transport write failures (for example, a disk-full condition
  when writing to `logs/combined.log`) are handled inside Winston and
  surfaced via Winston's own internal warning channels. They do not
  propagate out of `logger.info(...)` / `logger.warn(...)` /
  `logger.error(...)` calls and they do not crash the caller.
- Operators should monitor disk availability under `logs/`. When the
  disk fills, Winston will emit internal warnings and, depending on
  configuration and the underlying transport, may drop log lines.
- The service does not catch or rethrow Winston errors. Logging is a
  best-effort, fire-and-forget operation from the application's point of
  view.
...
- **Structured JSON log format limits some injection vectors.** The
  ...
  `src/utils/logger.js` lines 45-49; `src/utils/sanitizer.js`
  lines 29-33.
...
`src/utils/logger.js` lines 45-53.
...
- **The logger is a singleton.** `require('./utils/logger')` returns the
  ...
  `src/utils/logger.js` line 127 (`module.exports = logger`).
- **`logger.stream` is Morgan-specific.** The `{ write(message) }`
  ...
  `src/utils/logger.js` lines 110-114.
- **File rotation is bounded by Winston's built-in policy.** Each file
  ...
  Source: `src/utils/logger.js` lines 66-67, 77-78.
...
- **Log level cannot be changed at runtime.** The `config` object is
  ...
  the process. Source: `src/config/index.js` line 38
...
- **`logs/combined.log` is capped at the `http` level.** Even when
  ...
  console transport. Source: `src/utils/logger.js` line 65.
